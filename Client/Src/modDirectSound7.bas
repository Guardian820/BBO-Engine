...
Public Sub InitDirectSound()
    ...
    If Err.Number <> 0 Then
        Call MsgBox("Falha ao iniciar o Directx,não foi possível executar o DirectSound.")
        Exit Sub
    End If
...
Public Sub SoundPlay(ByVal File As String, Optional ByVal Volume As Long = 100, Optional ByVal Pan As Long = 50)
    ...
    If Not FileExist("SFX\" & File) Then
        Call AddText("Warning: Couldn't find 'SFX\" & File & "'!", BrightRed)
        Exit Sub
    End If
